    ...
    public static void plotTrip(int seconds, ArrayList<TripPoint> trip, JMapViewer map) throws IOException {
    	...
    	for (int i = 0; i < trip.size(); ++i) {
    		...
    		if (i != 0) {
                ...
                //get rotation angle
               double startLon = trip.get(i-1).getLon();
               double startLat = trip.get(i-1).getLat();
               double endLon = trip.get(i).getLon();
               double endLat = trip.get(i).getLat();

                angle = Math.toDegrees(Math.atan2(endLat - startLat, startLon - endLon)) - 90;
                

    		}
            //create a new image that has the rotated arrow
            BufferedImage newArrow = new BufferedImage(arrow.getWidth(), arrow.getHeight(), BufferedImage.TYPE_INT_ARGB);
            Graphics2D g = (Graphics2D) newArrow.getGraphics();
            AffineTransform transform = new AffineTransform();
            transform.translate(newArrow.getWidth()/2, newArrow.getHeight()/2);
    		// convert angle to radians
            transform.rotate(Math.toRadians(angle));
            transform.translate(-arrow.getWidth()/2, -arrow.getHeight()/2);
            g.drawImage(arrow, transform, null);
            g.dispose();

            marker = new IconMarker(c1, newArrow);
